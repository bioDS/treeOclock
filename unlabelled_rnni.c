...
// Author: Lena Collienneneighbours
...
Unlabelled_Tree_Array unlabelled_rnni_neighbourhood(Unlabelled_Tree *input_tree){
    ...
    for (long i = 0; i < neighbour_list.num_trees; i++){
        ...
        neighbour_list.trees[i].node_array = malloc((num_leaves - 1) * sizeof(Node));
    }
    ...
    neighbour->node_array = malloc((num_leaves - 1) * sizeof(Node)); // deep copy start tree
    for (long i = 0; i < num_leaves - 1; i++){
        neighbour->node_array[i] = input_tree->node_array[i];
    }
    ...
    for (long r=0; r<num_leaves-2; r++){
        ...
        if (input_tree->node_array[r].parent != r+1){
